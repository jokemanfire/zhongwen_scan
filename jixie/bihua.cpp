...
bool is_fittingTrue(int array1[][max_size], int array2[][max_size],int flag=1)
{
	...
	else
	{
		...
		std::cout << "一方程与二方程差:" << fabs(sume) / min(counts2,counts1) << endl;
		std::cout << "二方程与一方程差:" << fabs(sume3) / min(counts2, counts1) << endl;
		std::cout << "总差为:" << max(fabs(fabs(sume1) / counts1 - fabs(sume) / min(counts2, counts1)), fabs(fabs(sume2) / counts1 - fabs(sume3) / min(counts2, counts1))) << endl;
		*/
		if (fabs(sume1) / counts1 > 1||fabs(sume2)/counts2>1)
			return false;
		if (max(fabs(fabs(sume1) / counts1 - fabs(sume) / min(counts2, counts1)), fabs(fabs(sume2) / counts1 - fabs(sume3) / min(counts2, counts1))) <5) //设置误差阈值
			return true;
		else
			return false;
	}
...
void combine_bihua2(bi_hua * const yuan1, bi_hua * const yuan2,int flag)//对水平方向合并
{
	bi_hua * now_bi = yuan1;
